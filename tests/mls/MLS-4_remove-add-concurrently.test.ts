...
const TESTCASE_STEPS = [
   ...
   '3. Owner: removes Member1 and Admin1: removes Member2 and Admin2: removes Member3 concurrently',
...
const TESTCASE_NAME = `Remove and add member concurrently`;

const text = 'test MLS';
const GROUP = { name: GROUP_REF[2].name };

test.describe('[MLS Group][Remove --> Add member concurrently]', () => {
