...
const IDPrefix   = 'TRY-L';
const NamePrefix = 'Verify that User can reply on link message by';
...
runCases(
   {
      ...
      scope:         'once',
      skipPushNotif: true,      
      description:   ctx => `[Thread Conversation] Reply on link to ${ctx}`,
      ...
      prelude: async (sender, shared) => {
         await sender.actions.do('reply', { id: shared.rootId, type: Link.ROOT.type, value: Link.ROOT.value });
      },
      verifyQuote: true,
   },
   [
