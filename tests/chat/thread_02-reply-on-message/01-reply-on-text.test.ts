...
runCases(
   {
      ...
      scope:         'once',
      skipPushNotif: true,      
      description:   ctx => `[Thread Conversation] Reply on text to ${ctx}`,
      ...
      prelude: async (sender, shared) => {
         await sender.actions.do('reply', { id: shared.rootId, type: Text.ROOT.type, value: Text.ROOT.value });
      },
      verifyQuote: true,
   },
   [
