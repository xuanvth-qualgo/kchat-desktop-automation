...
const IDPrefix   = 'MCR-T';
const NamePrefix = 'Verify that User can create thread on text message by';

runCases(
   {
      feature:       '[Main Conversation] Create thread on message',
      severity:      'critical',
      scope:         'once',
      skipPushNotif: true,
      description:   ctx => `[Main Conversation] Create thread on text to ${ctx}`,
      ...
      prelude: async (sender, shared) => {
         await sender.actions.do('createThread', { id: shared.rootId, type: Text.ROOT.type, value: Text.ROOT.value });
      },
      verifyQuote: true,
   },
   [
