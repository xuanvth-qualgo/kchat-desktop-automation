...
/* EDIT ON: text, mention, link, emoji, image, video, file */
/* TOTAL: 20, SMOKE: 5, REGRESSION: 15 */
...
const TESTCASE_STEPS = [
   ...
   '4. User1: Select conversation >> Edit message, verify and get last edited message ID.', 
   '5. User2: Reopen app >> Select conversation >> Verify last received message >> Switch to other conversation.'
];

const ALLURE_BASE = { feature: 'Chat / Edit Message', severity: 'normal' as const, description: `Verify that User can edit and receive messages` };
const TESTCASE_NAME = (type: string) => `[Edit Message] Edit ${type} in 1 round`;
...
const mention2 = MENTION('autotest03').SHORT;
...
test.describe('[Chat][Edit Message]', () => {
   ...
   test(`[Direct Chat] ${TESTCASE_NAME('text')}`, { tag: '@smoke' }, async ({ chatUser1: u1, chatUser2: u2 }) => {
      ...
      await stepAndRetry(`${TESTCASE_STEPS[3]}`, async () => { await  CHAT.ACTION.actionOnMsgInConv(u1, { name: u2.username }, 'edit', sentMsgId, 'text', `EDITED ${text}`); });
      await stepAndRetry(`${TESTCASE_STEPS[4]}`, async () => {
         expUnread = oldUnread + newUnread; 
         await  CHAT.ACTION.verifyReceivedActionOnMsg(u2, { name: u1.username }, 'edit', 'text', `EDITED ${text}`, expUnread, sentMsgId);
      });
   });
   test(`[Group Chat] ${TESTCASE_NAME('text')}`, { tag: '@smoke' }, async ({ chatUser1: u1, chatUser2: u2 }) => {
      ...
      await stepAndRetry(`${TESTCASE_STEPS[3]}`, async () => { await  CHAT.ACTION.actionOnMsgInConv(u1, GROUP, 'edit', sentMsgId, 'text', `EDITED ${text}`); });
      await stepAndRetry(`${TESTCASE_STEPS[4]}`, async () => {
         expUnread = oldUnread + newUnread; 
         await  CHAT.ACTION.verifyReceivedActionOnMsg(u2, GROUP, 'edit', 'text', `EDITED ${text}`, expUnread, sentMsgId);
      });
   });
   test(`[Community Chat] ${TESTCASE_NAME('text')}`, { tag: '@smoke' }, async ({ chatUser1: u1, chatUser2: u2 }) => {
      ...
      await stepAndRetry(`${TESTCASE_STEPS[3]}`, async () => { await  CHAT.ACTION.actionOnMsgInConv(u1, COMM_CHANNEL, 'edit', sentMsgId, 'text', `EDITED ${text}`); });
      await stepAndRetry(`${TESTCASE_STEPS[4]}`, async () => {
         expUnread = oldUnread + newUnread; 
         await  CHAT.ACTION.verifyReceivedActionOnMsg(u2, COMM_CHANNEL, 'edit', 'text', `EDITED ${text}`, expUnread, sentMsgId);
      });
   });
   /*=======================================================*/   
   ...
   test(`[Group Chat] ${TESTCASE_NAME('mention')}`, { tag: '@smoke' }, async ({ chatUser1: u1, chatUser2: u2 }) => {
      ...
      await stepAndRetry(`${TESTCASE_STEPS[3]}`, async () => { await  CHAT.ACTION.actionOnMsgInConv(u1, GROUP, 'edit', sentMsgId, 'mention', `EDITED ${mention} ${mention2}`); });
      await stepAndRetry(`${TESTCASE_STEPS[4]}`, async () => {
         expUnread = oldUnread + newUnread; 
         await  CHAT.ACTION.verifyReceivedActionOnMsg(u2, GROUP, 'edit', 'mention', `EDITED ${mention} ${mention2}`, expUnread, sentMsgId);
      });
   });
   test(`[Community Chat] ${TESTCASE_NAME('mention')}`, { tag: '@smoke' }, async ({ chatUser1: u1, chatUser2: u2 }) => {
      ...
      await stepAndRetry(`${TESTCASE_STEPS[3]}`, async () => { await  CHAT.ACTION.actionOnMsgInConv(u1, COMM_CHANNEL, 'edit', sentMsgId, 'mention', `EDITED ${mention} ${mention2}`); });
      await stepAndRetry(`${TESTCASE_STEPS[4]}`, async () => {
         expUnread = oldUnread + newUnread; 
         await  CHAT.ACTION.verifyReceivedActionOnMsg(u2, COMM_CHANNEL, 'edit', 'mention', `EDITED ${mention} ${mention2}`, expUnread, sentMsgId);
      });
   });
   /*=======================================================*/   
